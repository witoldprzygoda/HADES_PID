...
void pid_pip_cut_pp45_exp() {
  ...
  TString drawCmd = Form(
    "pip_beta - pip_p/sqrt(pip_p*pip_p + %.2f*%.2f) : pip_p >> %s(400,0,2000,300,-0.15,0.15)",
  ...
  //chain->Draw(Form("pip_beta : pip_p >> %s(400,0,2000,300,0.0,1.2)", h2pb_name), "!protoncut && eVertReco_z>-500 && start_iteration==3", "colz");
  chain->Draw(Form("pip_beta : pip_p >> %s(400,0,2000,300,0.0,1.2)", h2pb_name), "eVertReco_z>-500 && start_iteration==3", "colz");
  ...
  */
  ...
  if (contours.valid) {
    saveContourResults(
        ...
        widthLabels,
        nWidths,
        "pip_pid_cuts_pp45_exp.root",              // Output filename
        "pip"                             // Particle name
    );
    ...
    //if (cCuts) {
    //  cCuts->SaveAs("pip_pid_cuts_overlay_pp45_exp.png");
    //}
  }
