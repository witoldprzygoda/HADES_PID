...
void pid_pim_cut_pp45_exp() {
  ...
  //chain->Draw(Form("pim_beta : pim_p >> %s(400,0,2000,300,0.0,1.2)", h2pb_name), "!protoncut && eVertReco_z>-500 && start_iteration==3", "colz");
  chain->Draw(Form("pim_beta : pim_p >> %s(400,0,2000,300,0.0,1.2)", h2pb_name), "eVertReco_z>-500 && start_iteration==3", "colz");
  ...
  */
  ...
  if (contours.valid) {
    saveContourResults(
        ...
        widthLabels,
        nWidths,
        "pim_pid_cuts_pp45_exp.root",              // Output filename
        "pim"                             // Particle name
    );
    ...
    //if (cCuts) {
    //  cCuts->SaveAs("pim_pid_cuts_overlay_pp45_exp.png");
    //}
  }
