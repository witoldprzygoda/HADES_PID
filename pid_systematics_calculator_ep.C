// =====================================================
// pid_systematics_calculator_ep.C
// =====================================================
// 
...
//
// USAGE:
//   root -l 'pid_systematics_calculator_ep.C("ep", "ep_syst_output.root")'
//
// =====================================================
...
// =====================================================
// GLOBAL CONFIGURATION - POSITRON (ep)
// =====================================================
const double gMomentumMax = 1600.0;  // Positron momentum range [MeV/c]
const double gPositronMass = 0.511;  // Positron mass [MeV/c²]
...
vector<SystematicResult> calculateYieldSystematics(
  ...
  cout << "  1. Use REFERENCE TCutG (ep_pid_cuts_exp.root) as your nominal selection" << endl;
...
void plotTCutGComparison(
  ...
  // Try both naming conventions: with and without width index
  TString cut3exp_name = Form("cut_%s_3sig", particleName.Data());
  TString cut5exp_name = Form("cut_%s_5sig", particleName.Data());
  ...
  if (fExp && !fExp->IsZombie()) {
    // Try without width index first (positron format)
    cut3exp = (TCutG*)fExp->Get(Form("TCutG/%s", cut3exp_name.Data()));
    cut5exp = (TCutG*)fExp->Get(Form("TCutG/%s", cut5exp_name.Data()));
    
    // If not found, try with width index
    if (!cut3exp) cut3exp = (TCutG*)fExp->Get(Form("TCutG/cut_%s_3sig_w%d", particleName.Data(), widthIdx));
    if (!cut5exp) cut5exp = (TCutG*)fExp->Get(Form("TCutG/cut_%s_5sig_w%d", particleName.Data(), widthIdx));
  ...
  if (fSim && !fSim->IsZombie()) {
    // Try without width index first
    cut3sim = (TCutG*)fSim->Get(Form("TCutG/cut_%s_3sig", particleName.Data()));
    cut5sim = (TCutG*)fSim->Get(Form("TCutG/cut_%s_5sig", particleName.Data()));
    
    // If not found, try with width index
    if (!cut3sim) cut3sim = (TCutG*)fSim->Get(Form("TCutG/cut_%s_3sig_w%d", particleName.Data(), widthIdx));
    if (!cut5sim) cut5sim = (TCutG*)fSim->Get(Form("TCutG/cut_%s_5sig_w%d", particleName.Data(), widthIdx));
  ...
  fTheory->SetParameter(0, gPositronMass);
  ...
  TH2F* frame3 = new TH2F("frame3", Form("%s: 3#sigma TCutG Comparison;p [MeV/c];#beta", particleName.Data()),
                          100, 0, gMomentumMax, 100, 0.0, 1.2);
  frame3->SetStats(0);
  frame3->Draw();
  ...
  leg3->SetFillStyle(0);
  if (cut3exp) leg3->AddEntry(cut3exp, "Exp (Reference) 3#sigma", "l");
  if (cut3sim) leg3->AddEntry(cut3sim, "Sim 3#sigma", "l");
  ...
  tex3.DrawLatex(0.15, 0.88, "e^{+}: 3#sigma comparison");
  ...
  leg5->SetFillStyle(0);
  if (cut5exp) leg5->AddEntry(cut5exp, "Exp (Reference) 5#sigma", "l");
  if (cut5sim) leg5->AddEntry(cut5sim, "Sim 5#sigma", "l");
  ...
  tex5.DrawLatex(0.15, 0.88, "e^{+}: 5#sigma comparison");
  
  cComp->SaveAs(Form("%s_tcutg_comparison.png", outputPrefix.Data()));
}
...
void calculateSimMismatch(
  ...
  if (!hasSim) {
    cout << "[INFO] No simulation samples loaded - skipping mismatch analysis." << endl;
    return;
  }
  
  cout << string(80, '-') << endl;
  cout << Form("%-10s %12s %12s %12s %12s %12s", 
               "p [MeV/c]", "σ_exp", "σ_sim", "ratio", "ε_exp", "Δε [%]") << endl;
  cout << string(80, '-') << endl;
  ...
  for (const auto& cp : comparison) {
    if (cp.sigmaSim.empty() || cp.nExpSamples < 1) continue;
    
    double sigmaExp = cp.sigmaAvg;
    double sigmaSim = cp.sigmaSim[0];
    
    if (sigmaExp <= 0 || sigmaSim <= 0) continue;
    
    double ratio = sigmaSim / sigmaExp;
    double effExp = 2.0 * gaussianCDF(Nsigma) - 1.0;
    double effMismatch = calcEfficiency(0, ratio, 0, 1.0, Nsigma);
    double deltaEff = effMismatch - effExp;
    
    totalMismatch += std::abs(deltaEff);
    if (std::abs(deltaEff) > maxMismatch) {
      maxMismatch = std::abs(deltaEff);
      pAtMaxMismatch = cp.p;
    }
    nPoints++;
    
    if (((int)cp.p % 100) < 15) {
      cout << Form("%-10.0f %12.5f %12.5f %12.3f %12.6f %12.4f",
                   cp.p, sigmaExp, sigmaSim, ratio, effExp, 100*deltaEff) << endl;
    }
  }
...
vector<FitPoint> loadFitResults(const TString& filename, int widthIdx = 1, bool debug = false, bool isSimulation = false) {
  ...
  for (Long64_t i = 0; i < nEntries; ++i) {
    ...
    // Positron sigma limits (narrower than pion due to ultra-relativistic nature)
    if (sigma < 0.001 || sigma > 0.05) { 
      if (debug && nBadSigma < 5) cout << "    [DEBUG] sigma=" << sigma << " at p=" << p << endl;
      nBadSigma++; continue; 
    }
    if (std::abs(mean) > 0.03) { 
  ...
  if (results.size() < 100 || debug) {
    ...
    cout << "    Filtered by sigma out of [0.001,0.05]: " << nBadSigma << endl;
    cout << "    Filtered by |mean|>0.03: " << nBadMean << endl;
...
TCutG* loadTCutG(const TString& filename, const TString& cutName) {
  ...
  // If not found, try without TCutG/ prefix
  if (!cut) cut = (TCutG*)f->Get(cutName);
...
TCutG* generateEnvelopeCut(const vector<TCutG*>& cuts, const TString& cutName,
  ...
  for (double p = pMin; p <= pMax; p += pStep) {
    ...
    for (const auto& cut : cuts) {
      ...
      for (int i = 0; i < n; ++i) {
        if (std::abs(x[i] - p) < pStep/2.0) {
          if (y[i] > maxBeta) maxBeta = y[i];
          if (y[i] < minBeta) minBeta = y[i];
        }
      }
    }
    
    if (maxBeta > -1e8 && minBeta < 1e8) {
      pPoints.push_back(p);
      betaUpper.push_back(maxBeta);
      betaLower.push_back(minBeta);
    }
  }
  ...
  for (size_t i = 0; i < pPoints.size(); ++i) {
    cutX.push_back(pPoints[i]);
    cutY.push_back(betaUpper[i]);
  }
  for (int i = pPoints.size() - 1; i >= 0; --i) {
    cutX.push_back(pPoints[i]);
    cutY.push_back(betaLower[i]);
  }
  cutX.push_back(cutX[0]);
  cutY.push_back(cutY[0]);
  
  TCutG* envCut = new TCutG(cutName, cutX.size(), cutX.data(), cutY.data());
  envCut->SetVarX("ep_p");
  envCut->SetVarY("ep_beta");
  envCut->SetLineColor(kMagenta);
  envCut->SetLineWidth(2);
...
void createComparisonPlots(
  ...
  mgMean->Draw("A");
  mgMean->GetYaxis()->SetRangeUser(-0.015, 0.015);
  ...
  gMeanBand->Draw("A3");
  gMeanBand->GetYaxis()->SetRangeUser(-0.015, 0.015);
  ...
  if (hasSim) {
    ...
    cDS->SaveAs(Form("%s_data_sim_comparison.png", outputPrefix.Data()));
  }
...
void printSummaryTable(const vector<SampleInfo>& samples, const vector<ComparisonPoint>& comparison,
                       const TString& particleName) {
  cout << "\n" << string(70, '=') << "\n  SYSTEMATIC SUMMARY: " << particleName << " (e+)" << "\n" << string(70, '=') << endl;
...
// =====================================================
// MAIN FUNCTION - POSITRON (ep)
// =====================================================
void pid_systematics_calculator_ep(
    const TString& particleName = "ep",
    const TString& outputFile = "pid_ep_systematics.root",
    int widthIdx = 1
) {
  cout << "\n" << string(60, '=') << "\n  PID SYSTEMATICS CALCULATOR (e+)" << "\n" << string(60, '=') << endl;
  
  // =====================================================
  // CONFIGURE YOUR INPUT FILES HERE - POSITRON (ep)
  // =====================================================
  vector<SampleInfo> samples;
  
  // EXPERIMENTAL (included in systematics)
  samples.push_back({"ep_pid_cuts_exp.root", "Exp (Ref)", "temporal", kReference, kRed, 20, 1});
  samples.push_back({"ep_pid_cuts_exp.root", "Exp2", "temporal", kExperimental, kBlue, 21, 1});
  
  // SIMULATION (comparison only - NOT in systematics!)
  samples.push_back({"ep_pid_cuts_sim.root", "Simulation", "simulation", kSimulation, kBlack, 25, 2});
  ...
  // Envelope cuts (experimental only) - Positron mass
  ...
  for (size_t s = 0; s < valid.size(); ++s) {
    ...
    // Try cut names without width index first (positron format from our macros)
    TString n1 = Form("cut_%s_1sig", particleName.Data());
    TString n2p5 = Form("cut_%s_25sig", particleName.Data());
    TString n3 = Form("cut_%s_3sig", particleName.Data());
    TString n3p5 = Form("cut_%s_35sig", particleName.Data());
    TString n5 = Form("cut_%s_5sig", particleName.Data());
    
    // Load all sigma levels
    TCutG* t1 = loadTCutG(valid[s].filename, n1);
    TCutG* t2p5 = loadTCutG(valid[s].filename, n2p5);
    TCutG* t3 = loadTCutG(valid[s].filename, n3);
    TCutG* t3p5 = loadTCutG(valid[s].filename, n3p5);
    TCutG* t5 = loadTCutG(valid[s].filename, n5);
    
    // If not found, try with width index
    if (!t1) t1 = loadTCutG(valid[s].filename, Form("cut_%s_1sig_w%d", particleName.Data(), widthIdx));
    if (!t2p5) t2p5 = loadTCutG(valid[s].filename, Form("cut_%s_25sig_w%d", particleName.Data(), widthIdx));
    if (!t3) t3 = loadTCutG(valid[s].filename, Form("cut_%s_3sig_w%d", particleName.Data(), widthIdx));
    if (!t3p5) t3p5 = loadTCutG(valid[s].filename, Form("cut_%s_35sig_w%d", particleName.Data(), widthIdx));
    if (!t5) t5 = loadTCutG(valid[s].filename, Form("cut_%s_5sig_w%d", particleName.Data(), widthIdx));
    
    if (t1) c1.push_back(t1);
    if (t2p5) c2p5.push_back(t2p5);
    if (t3) c3.push_back(t3);
    if (t3p5) c3p5.push_back(t3p5);
    if (t5) c5.push_back(t5);
  ...
  TCutG* env1 = generateEnvelopeCut(c1, Form("envelope_%s_1sig", particleName.Data()), gPositronMass, 10, gMomentumMax);
  TCutG* env2p5 = generateEnvelopeCut(c2p5, Form("envelope_%s_25sig", particleName.Data()), gPositronMass, 10, gMomentumMax);
  TCutG* env3 = generateEnvelopeCut(c3, Form("envelope_%s_3sig", particleName.Data()), gPositronMass, 10, gMomentumMax);
  TCutG* env3p5 = generateEnvelopeCut(c3p5, Form("envelope_%s_35sig", particleName.Data()), gPositronMass, 10, gMomentumMax);
  TCutG* env5 = generateEnvelopeCut(c5, Form("envelope_%s_5sig", particleName.Data()), gPositronMass, 10, gMomentumMax);
  ...
  cout << "\n\n";
  cout << string(80, '#') << endl;
  cout << "#  DETAILED SYSTEMATIC ANALYSIS (e+)" << endl;
  cout << string(80, '#') << endl;
