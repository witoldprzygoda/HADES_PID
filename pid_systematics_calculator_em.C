// =====================================================
// pid_systematics_calculator_em.C
// =====================================================
// 
...
//
// USAGE:
//   root -l 'pid_systematics_calculator_em.C("em", "em_syst_output.root")'
//
// =====================================================
...
// =====================================================
// GLOBAL CONFIGURATION - ELECTRON (em)
// =====================================================
...
vector<SystematicResult> calculateYieldSystematics(
  ...
  cout << "  1. Use REFERENCE TCutG (em_pid_cuts_exp.root) as your nominal selection" << endl;
...
void plotTCutGComparison(
  ...
  if (fExp && !fExp->IsZombie()) {
    // Try without width index first (electron format)
...
// =====================================================
// MAIN FUNCTION - ELECTRON (em)
// =====================================================
void pid_systematics_calculator_em(
    const TString& particleName = "em",
    const TString& outputFile = "pid_em_systematics.root",
    int widthIdx = 1
) {
  cout << "\n" << string(60, '=') << "\n  PID SYSTEMATICS CALCULATOR (e-)" << "\n" << string(60, '=') << endl;
  
  // =====================================================
  // CONFIGURE YOUR INPUT FILES HERE - ELECTRON (em)
  // =====================================================
  vector<SampleInfo> samples;
  
  // EXPERIMENTAL (included in systematics)
  samples.push_back({"em_pid_cuts_exp.root", "Exp (Ref)", "temporal", kReference, kRed, 20, 1});
  samples.push_back({"em_pid_cuts_exp.root", "Exp2", "temporal", kExperimental, kBlue, 21, 1});
  
  // SIMULATION (comparison only - NOT in systematics!)
  samples.push_back({"em_pid_cuts_sim.root", "Simulation", "simulation", kSimulation, kBlack, 25, 2});
  ...
  for (size_t s = 0; s < valid.size(); ++s) {
    ...
    // Try cut names without width index first (electron format from our macros)
