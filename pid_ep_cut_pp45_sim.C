...
// =====================================================
// PHYSICS FUNCTIONS - POSITRON
// =====================================================

double betaPositron(double p) {
  return p / std::sqrt(p * p + gPositronMass * gPositronMass);
}

double deltaBetaToBeta(double p, double dBeta) {
  return betaPositron(p) + dBeta;
}
...
// =====================================================
// ROBUST FITTING FUNCTION - POSITRON
// =====================================================
...
void pid_ep_cut_pp45_sim() {
  gROOT->SetBatch(kFALSE);
  gStyle->SetOptStat(0);
  gStyle->SetOptFit(111);

  // --- Build TChain for POSITRON data ---
  const char* treeName = "Ep_ID";
  ...
  chain->Draw(drawCmd, "eVertReco_z>-500 && ep_p>0 && ep_beta>0 && ep_beta<1.5 && ep_sim_id==2", "colz");
  ...
  h2DB->SetTitle("Positron: Momentum vs #Delta#beta (#beta - #beta_{e^{+}})");
  ...
  chain->Draw(Form("ep_beta : ep_p >> %s(400,0,1600,300,0.0,1.2)", h2pb_name), 
              "eVertReco_z>-500 && ep_p>0 && ep_beta>0 && ep_beta<1.5 && ep_sim_id==2", "colz");
  TH2F* h2PB = static_cast<TH2F*>(gDirectory->Get(h2pb_name));
  if (h2PB) {
    h2PB->SetTitle("Positron: #beta vs Momentum");
  ...
  const char* h2ma_name = "h2_ep_mass_a";
  TString drawMA = Form(
    "sqrt(1 + %.1e*ep_p*ep_p - pow(1-ep_beta*ep_beta,2)) : "
    "ep_p*sqrt(1/pow(ep_beta,2) - 1) >> %s(300,0,300,160,0,16)",
    gSSquared, h2ma_name);
  if (gDirectory->FindObject(h2ma_name)) gDirectory->Delete(Form("%s;*", h2ma_name));
  chain->Draw(drawMA, "eVertReco_z>-500 && ep_p>0 && ep_beta>0 && ep_beta<1.5 && ep_sim_id==2", "colz");
  TH2F* h2MA = static_cast<TH2F*>(gDirectory->Get(h2ma_name));
  if (h2MA) {
    h2MA->SetTitle("Positron: a-parameter vs Mass");
  ...
  chain->Draw(Form("ep_p*ep_p*(1.0/(ep_beta*ep_beta) - 1) : ep_p >> %s(400,0,1600,400,-20000,60000)", h2m2_name), 
              "eVertReco_z>-500 && ep_p>0 && ep_beta>0.1 && ep_beta<1.5 && ep_sim_id==2", "colz");
  TH2F* h2M2 = static_cast<TH2F*>(gDirectory->Get(h2m2_name));
  if (h2M2) {
    h2M2->SetTitle("Positron: Mass^{2} vs Momentum");
    h2M2->GetXaxis()->SetTitle("Momentum [MeV/c]");
    h2M2->GetYaxis()->SetTitle("Mass^{2} [MeV^{2}/c^{4}]");
  }

  // =====================================================
  // SCANNING PARAMETERS - POSITRON
  ...
  for (int w = 0; w < nWidths; ++w) {
    ...
    cFits[w] = new TCanvas(Form("c_fits_dBeta_ep_%d", w), 
                           Form("Delta-Beta Fits (Positron e^{+}) - %s", widthLabels[w].Data()), 1400, 900);
    cFits[w]->Divide(4, 3);
    ...
    cFits[w]->Modified();
    cFits[w]->Update();
    cFits[w]->SaveAs(Form("ep_fits_%s.png", widthLabels[w].Data()));
  }
  ...
  TCanvas* cCombDB_1sig = new TCanvas("c_comb_db_1sig_ep", "Combined 1#sigma in #Delta#beta (Positron)", 1000, 800);
  cCombDB_1sig->cd();
  h2DB->Draw("colz");
  ...
  cCombDB_1sig->Update();
  cCombDB_1sig->SaveAs("ep_contours_db_1sig.png");

  // Combined 3σ in Δβ
  TCanvas* cCombDB_3sig = new TCanvas("c_comb_db_3sig_ep", "Combined 3#sigma in #Delta#beta (Positron)", 1000, 800);
  cCombDB_3sig->cd();
  h2DB->Draw("colz");
  ...
  cCombDB_3sig->Update();
  cCombDB_3sig->SaveAs("ep_contours_db_3sig.png");
  ...
  // Combined 1σ in (p, β)
  TCanvas* cCombPB_1sig = new TCanvas("c_comb_pb_1sig_ep", "Combined 1#sigma in (p, #beta) (Positron)", 1000, 800);
  cCombPB_1sig->cd();
  if (h2PB) h2PB->Draw("colz");
  
  TF1* epCurvePB1 = new TF1("epCurvePB1", Form("x/sqrt(x*x + %f*%f)", gPositronMass, gPositronMass), 0, gExtendTo);
  epCurvePB1->SetLineColor(kBlack);
  epCurvePB1->SetLineStyle(kDashed);
  epCurvePB1->SetLineWidth(2);
  epCurvePB1->Draw("same");
  ...
  for (int w = 0; w < nWidths; ++w) {
    ...
    for (size_t i = 0; i < rawP.size(); ++i) {
      double p = rawP[i];
      double beta_ep = betaPositron(p);
      
      double beta_upper = beta_ep + smoothMean[i] + 1.0 * smoothSigma[i];
      if (beta_upper > 0.1 && beta_upper < 1.15) {
        pPoints1Upper.push_back(p);
        betaPoints1Upper.push_back(beta_upper);
      }
      
      double beta_lower = beta_ep + smoothMean[i] - 1.0 * smoothSigma[i];
  ...
  legPB_1->AddEntry(epCurvePB1, "m_{e}=0.511", "l");
  legPB_1->Draw();
  cCombPB_1sig->Modified();
  cCombPB_1sig->Update();
  cCombPB_1sig->SaveAs("ep_contours_pb_1sig.png");

  // Combined 3σ in (p, β)
  TCanvas* cCombPB_3sig = new TCanvas("c_comb_pb_3sig_ep", "Combined 3#sigma in (p, #beta) (Positron)", 1000, 800);
  cCombPB_3sig->cd();
  if (h2PB) h2PB->Draw("colz");
  
  // Positron β(p) theory curve
  TF1* epCurvePB3 = new TF1("epCurvePB3", Form("x/sqrt(x*x + %f*%f)", gPositronMass, gPositronMass), 0, gExtendTo);
  epCurvePB3->SetLineColor(kBlack);
  epCurvePB3->SetLineStyle(kDashed);
  epCurvePB3->SetLineWidth(2);
  epCurvePB3->Draw("same");
  ...
  for (int w = 0; w < nWidths; ++w) {
    ...
    for (size_t i = 0; i < rawP.size(); ++i) {
      ...
      double beta_upper = beta_ep + smoothMean[i] + 3.0 * smoothSigma[i];
      if (beta_upper > 0.1 && beta_upper < 1.15) {
        pPoints3Upper.push_back(p);
        betaPoints3Upper.push_back(beta_upper);
      }
      
      double beta_lower = beta_ep + smoothMean[i] - 3.0 * smoothSigma[i];
      if (beta_lower > 0.1 && beta_lower < 1.15) {
  ...
  legPB_3->AddEntry(epCurvePB3, "m_{e}=0.511", "l");
  legPB_3->Draw();
  cCombPB_3sig->Modified();
  cCombPB_3sig->Update();
  cCombPB_3sig->SaveAs("ep_contours_pb_3sig.png");
  ...
  const double epMass2 = gPositronMass * gPositronMass;  // ~0.26 MeV²/c⁴
  
  // Combined 1σ in (p, mass²)
  TCanvas* cCombM2_1sig = new TCanvas("c_comb_m2_1sig_ep", "Combined 1#sigma in (p, mass^{2}) (Positron)", 1000, 800);
  cCombM2_1sig->cd();
  if (h2M2) h2M2->Draw("colz");
  
  TLine* epLineM2_1 = new TLine(0, epMass2, gExtendTo, epMass2);
  epLineM2_1->SetLineColor(kBlack);
  epLineM2_1->SetLineStyle(kDashed);
  epLineM2_1->SetLineWidth(2);
  epLineM2_1->Draw("same");
  ...
  legM2_1->AddEntry(epLineM2_1, "m_{e}^{2}=0.26", "l");
  legM2_1->Draw();
  cCombM2_1sig->Modified();
  cCombM2_1sig->Update();
  cCombM2_1sig->SaveAs("ep_contours_m2_1sig.png");

  // Combined 3σ in (p, mass²)
  TCanvas* cCombM2_3sig = new TCanvas("c_comb_m2_3sig_ep", "Combined 3#sigma in (p, mass^{2}) (Positron)", 1000, 800);
  cCombM2_3sig->cd();
  if (h2M2) h2M2->Draw("colz");
  
  TLine* epLineM2 = new TLine(0, epMass2, gExtendTo, epMass2);
  epLineM2->SetLineColor(kBlack);
  epLineM2->SetLineStyle(kDashed);
  epLineM2->SetLineWidth(2);
  epLineM2->Draw("same");
  ...
  legM2_3->AddEntry(epLineM2, "m_{e}^{2}=0.26", "l");
  legM2_3->Draw();
  cCombM2_3sig->Modified();
  cCombM2_3sig->Update();
  cCombM2_3sig->SaveAs("ep_contours_m2_3sig.png");
  ...
  TCanvas* cCombMA_1sig = new TCanvas("c_comb_ma_1sig_ep", "Combined 1#sigma in (mass, a) (Positron)", 1000, 800);
  cCombMA_1sig->cd();
  if (h2MA) h2MA->Draw("colz");
  
  TLine* epLineMA_1 = new TLine(gPositronMass, 0, gPositronMass, 16);
  epLineMA_1->SetLineColor(kBlack);
  epLineMA_1->SetLineStyle(kDashed);
  epLineMA_1->SetLineWidth(2);
  epLineMA_1->Draw("same");
  ...
  legMA_1->AddEntry(epLineMA_1, "m_{e}=0.511", "l");
  legMA_1->Draw();
  cCombMA_1sig->Modified();
  cCombMA_1sig->Update();
  cCombMA_1sig->SaveAs("ep_contours_ma_1sig.png");

  // Combined 3σ in (mass, a)
  TCanvas* cCombMA_3sig = new TCanvas("c_comb_ma_3sig_ep", "Combined 3#sigma in (mass, a) (Positron)", 1000, 800);
  cCombMA_3sig->cd();
  if (h2MA) h2MA->Draw("colz");
  
  TLine* epLineMA = new TLine(gPositronMass, 0, gPositronMass, 16);
  epLineMA->SetLineColor(kBlack);
  epLineMA->SetLineStyle(kDashed);
  epLineMA->SetLineWidth(2);
  epLineMA->Draw("same");
  ...
  legMA_3->AddEntry(epLineMA, "m_{e}=0.511", "l");
  legMA_3->Draw();
  cCombMA_3sig->Modified();
  cCombMA_3sig->Update();
  cCombMA_3sig->SaveAs("ep_contours_ma_3sig.png");

  // =====================================================
  // PARAMETER PLOTS
  // =====================================================
  
  TCanvas* cPar = new TCanvas("c_par_ep", "Fit Parameters vs Momentum (Positron)", 1200, 800);
  cPar->Divide(2, 2);
  ...
  if (rawP.size() >= 5) {
    ...
    auto makeCut = [&](double nSig, const char* cutName) -> TCutG* {
      ...
      for (size_t i = 0; i < rawP.size(); ++i) {
        ...
        if (p < 0) continue;
        double beta_ep = betaPositron(p);
        double beta_upper = beta_ep + smoothMean[i] + nSig * smoothSigma[i];
      ...
      for (int i = rawP.size() - 1; i >= 0; --i) {
        ...
        if (p < 0) continue;
        double beta_ep = betaPositron(p);
        double beta_lower = beta_ep + smoothMean[i] - nSig * smoothSigma[i];
    ...
    cut1sig  = makeCut(1.0, "cut_ep_1sig_w1");
    cut25sig = makeCut(2.5, "cut_ep_25sig_w1");
    cut3sig  = makeCut(3.0, "cut_ep_3sig_w1");
    cut35sig = makeCut(3.5, "cut_ep_35sig_w1");
    cut5sig  = makeCut(5.0, "cut_ep_5sig_w1");
  ...
  TCanvas* cCuts = new TCanvas("c_ep_cuts", "Positron PID Cuts in (p, #beta)", 1000, 800);
  cCuts->cd();
  
  if (h2PB) h2PB->Draw("colz");
  
  // Theory curve: β = p/√(p² + m²)
  TF1* theoryCurve = new TF1("theoryCurve_ep", 
    Form("x/sqrt(x*x + %f*%f)", gPositronMass, gPositronMass), 0, gExtendTo);
  ...
  legCuts->SetHeader(Form("Positron e^{+} (width %s)", widthLabels[selectedWidth].Data()));
  ...
  legCuts->AddEntry(theoryCurve, Form("m_{e}=%.3f MeV", gPositronMass), "l");
  legCuts->Draw();
  
  cCuts->Modified();
  cCuts->Update();
  cCuts->SaveAs("ep_pid_cuts_overlay.png");
  ...
  TFile* fOut = new TFile("ep_pid_cuts_pp45_sim.root", "RECREATE");
  ...
  if (cut1sig && cut25sig && cut3sig && cut35sig && cut5sig) {
    ...
    cout << "  - TCutG objects: cut_ep_1sig, cut_ep_25sig, cut_ep_3sig, cut_ep_35sig, cut_ep_5sig" << endl;
  }
  
  // Save metadata
  fOut->cd();
  TNamed* metaParticle = new TNamed("particle", "positron");
  TNamed* metaMass = new TNamed("mass_MeV", Form("%.3f", gPositronMass));
  ...
  cout << "[TCutG] Output file 'ep_pid_cuts_pp45_sim.root' saved successfully." << endl;

  // =====================================================
  // SUMMARY
  // =====================================================
  cout << "\n=========================================" << endl;
  cout << "=== POSITRON PID ANALYSIS COMPLETE ===" << endl;
  cout << "=========================================" << endl;
  cout << "Particle: e+ (positron), mass = " << gPositronMass << " MeV/c²" << endl;
  ...
  cout << "\nOutput files:" << endl;
  for (int w = 0; w < nWidths; ++w) {
    cout << "  - ep_fits_" << widthLabels[w] << ".png" << endl;
  }
  cout << "  - ep_contours_db_1sig.png  (Δβ, 1σ)" << endl;
  cout << "  - ep_contours_db_3sig.png  (Δβ, 3σ)" << endl;
  cout << "  - ep_contours_pb_1sig.png  (p,β, 1σ)" << endl;
  cout << "  - ep_contours_pb_3sig.png  (p,β, 3σ)" << endl;
  cout << "  - ep_contours_m2_1sig.png  (p,m², 1σ)" << endl;
  cout << "  - ep_contours_m2_3sig.png  (p,m², 3σ)" << endl;
  cout << "  - ep_contours_ma_1sig.png  (m,a, 1σ)" << endl;
  cout << "  - ep_contours_ma_3sig.png  (m,a, 3σ)" << endl;
  cout << "  - ep_parameters.png" << endl;
  cout << "  - ep_pid_cuts_overlay.png  <-- TCutG (1σ, 3σ, 5σ) on (p, β)" << endl;
  cout << "  - ep_pid_cuts_pp45_sim.root:" << endl;
