...
void pid_pip_cut_pp158_exp() {
  ...
  TString drawCmd = Form(
    "pip_beta - pip_p/sqrt(pip_p*pip_p + %.2f*%.2f) : pip_p >> %s(280,0,1400,300,-0.15,0.15)",
  ...
  //chain->Draw(Form("pip_beta : pip_p >> %s(280,0,1400,300,0.0,1.2)", h2pb_name), "!protoncut && eVertReco_z>-500 && start_iteration==3", "colz");
  chain->Draw(Form("pip_beta : pip_p >> %s(280,0,1400,300,0.0,1.2)", h2pb_name), "eVertReco_z>-500 && start_iteration==3", "colz");
  TH2F* h2PB = static_cast<TH2F*>(gDirectory->Get(h2pb_name));

  // (mass, a) histogram
  const char* h2ma_name = "h2_mass_a";
  TString drawMA = Form(
    "sqrt(1 + %.1e*pip_p*pip_p - pow(1-pip_beta*pip_beta,2)) : "
    "pip_p*sqrt(1/pow(pip_beta,2) - 1) >> %s(300,0,300,160,0,16)",
    gSSquared, h2ma_name);
  if (gDirectory->FindObject(h2ma_name)) gDirectory->Delete(Form("%s;*", h2ma_name));
  chain->Draw(drawMA, "!protoncut && eVertReco_z>-500 && start_iteration==3 && pip_beta>0 && pip_beta<1.5", "colz");
  ...
  chain->Draw(Form("pip_p*pip_p*(1.0/(pip_beta*pip_beta) - 1) : pip_p >> %s(280,0,1400,400,-20000,60000)", h2m2_name), 
              "!protoncut && eVertReco_z>-500 && start_iteration==3 && pip_beta>0.1 && pip_beta<1.5", "colz");
  ...
  const double endMom = 1100.0;          // Fit range for pions (extrapolate to 1400)
  ...
  */
  ...
  if (contours.valid) {
    saveContourResults(
        ...
        widthLabels,
        nWidths,
        "pip_pid_cuts_pp158_exp.root",              // Output filename
        "pip"                             // Particle name
    );
    ...
    //if (cCuts) {
    //  cCuts->SaveAs("pip_pid_cuts_overlay_pp158_exp.png");
    //}
  }
  ...
  cout << "\n=== TCutG OUTPUT ===" << endl;
  cout << "  ROOT file: pip_pid_cuts_exp.root" << endl;
